...
export default async function Parents({ categoryId }) {
  ...
  if (isObjEmpty(parentData)) {
    content = (
      <div className="flex flex-col justify-around items-center text-sm w-screen bg-neutral-200 dark:bg-neutral-800 text-neutral-800 dark:text-neutral-200 h-fit">
        <h1>Empty</h1>
      </div>
    );
  } else {
    content = parentData.map((parent) => {
      return (
        <div
          key={parent.id}
          className="flex flex-col items-center md:items-start text-sm w-full bg-neutral-200 dark:bg-neutral-800 text-neutral-800 dark:text-neutral-200"
        >
          <Link
